...
const KEYBOARD_LAYOUT = [
  ...
  [
    { label: "Mayús", value: "shift", className: "is-wide is-shift" },
    { label: "Z", value: "z" },
    { label: "X", value: "x" },
    { label: "C", value: "c" },
    { label: "V", value: "v" },
  ...
  [
    { label: "Espacio", value: " ", className: "is-space" }
  ]
];
...
const elements = {
  ...
  nextKeyText: document.querySelector("#nextKeyText"),
...
function buildWordQueue() {
  let queue = shuffle(state.wordBank);
...
function readSettings(form) {
  ...
  const customWords = parseCustomWords(String(formData.get("customWords") || ""));

  if (!Number.isInteger(wordCount) || wordCount < 1 || wordCount > 80) {
    setWordHint("Elegí una cantidad entre 1 y 80.", true);
    elements.wordCount.focus();
    return null;
  }

  if (customWords.length < wordCount) {
    const missing = wordCount - customWords.length;
    const plural = missing === 1 ? "palabra o frase más" : "palabras o frases más";
    setWordHint(`Faltan ${missing} ${plural} para iniciar la carrera.`, true);
    elements.customWords.focus();
    return null;
  }

  setWordHint(`Listo: se usarán las primeras ${wordCount} palabras o frases cargadas.`, false);

  return {
    playerName: cleanName || "Jugador",
    duration: Number(formData.get("duration")),
    aiWpm: Number(formData.get("difficulty")),
    wordsToWin: wordCount,
    wordBank: customWords.slice(0, wordCount)
  };
}

function parseCustomWords(rawWords) {
  return rawWords
    .split(/\r?\n/)
    .map((word) => word.trim())
    .filter(Boolean);
...
function setWordHint(message, isError = false) {
  elements.wordHint.textContent = message;
  elements.wordHint.classList.toggle("is-error", isError);
}
...
function resetRace(settings = {}) {
  Object.assign(state, {
    ...
    correctCharacters: 0,
    wordsWithErrors: 0,
    wordsToWin: settings.wordsToWin || 20,
    currentWordHasError: false,
    currentTarget: "",
    wordBank: settings.wordBank || DEFAULT_WORD_BANK,
    wordQueue: [],
    lastWord: "",
    animationId: null,
    isRunning: false
...
function renderProgress() {
  ...
  elements.wordsText.textContent = `${state.wordsCompleted}/${state.wordsToWin}`;
...
function renderKeyboard() {
  elements.keyboard.innerHTML = KEYBOARD_LAYOUT
    .map((row) => {
      const keyCount = row.reduce((total, key) => total + getKeySpan(key), 0);
      const keys = row
        .map((key) => `
          <span class="key ${key.className || ""}" data-key="${escapeHtml(key.value)}">
            ${escapeHtml(key.label)}
          </span>
        `)
        .join("");

      return `<div class="keyboard-row" style="--key-count: ${keyCount}">${keys}</div>`;
    })
    .join("");
}

function getKeySpan(key) {
  if (!key.className) {
    return 1;
  }

  if (key.className.includes("is-space")) {
    return 5;
  }

  if (key.className.includes("is-wider")) {
    return 3;
  }

  if (key.className.includes("is-wide")) {
    return 2;
  }
...
function updateKeyboardGuide(value) {
  const nextCharacter = [...state.currentTarget][[...value].length] || "";
  const normalizedKey = normalizeKey(nextCharacter);
  const needsShift = nextCharacter !== nextCharacter.toLocaleLowerCase("es-AR");

  elements.keyboard.querySelectorAll(".key").forEach((key) => {
    const keyValue = key.dataset.key;
    const shouldHighlight = keyValue === normalizedKey || (needsShift && keyValue === "shift");
    key.classList.toggle("is-next", shouldHighlight);
  });

  if (!nextCharacter) {
    elements.nextKeyText.textContent = "Palabra completa";
    return;
  }

  const label = nextCharacter === " " ? "Espacio" : nextCharacter;
  elements.nextKeyText.textContent = `Próxima tecla: ${label}`;
}

function normalizeKey(character) {
  const lowerCharacter = character.toLocaleLowerCase("es-AR");

  if (["¿", "?"].includes(lowerCharacter)) {
    return "?";
  }
  ...
  if (["á", "é", "í", "ó", "ú", "ü", "ñ"].includes(lowerCharacter)) {
...
function handleTyping(event) {
  ...
  updateKeyboardGuide(elements.typingInput.value);
...
function completeWord() {
  ...
  state.playerProgress = Math.min(100, state.wordsCompleted * getPlayerStep());
  elements.typingInput.value = "";
  state.currentTarget = getNextWord();
...
function calculateAiProgress(elapsedMs) {
  ...
  return Math.min(100, theoreticalWords * getPlayerStep());
}
...
function calculateMetrics() {
  ...
  return {
    ...
    expectedWords: state.wordsToWin,
    completionPercentage: (state.wordsCompleted / state.wordsToWin) * 100
  };
}
...
function renderResults(winner, metrics) {
  ...
  elements.metricsGrid.innerHTML = [
    ...
    ["Palabras o líneas con error", `${metrics.wordsWithErrors}`],
    ["Palabras o líneas correctas", `${metrics.wordsCompleted}`],
    ["Cantidad de palabras o líneas esperadas", `${metrics.expectedWords}`],
    ["Porcentaje de palabras o líneas escritas", `${metrics.completionPercentage.toFixed(1)}%`]
  ]
    .map(([label, value]) => `
      <div class="metric">
        <span>${label}</span>
...
function init() {
  elements.customWords.value = DEFAULT_WORD_BANK.join("\n");
  renderKeyboard();
  renderRecord();
  resetRace();
  renderProgress();
  ...
  elements.wordCount.addEventListener("input", () => {
    const count = Number(elements.wordCount.value) || 0;
    setWordHint(`Cargá al menos ${count || 1} palabras o frases, una por línea.`, false);
  });
  elements.customWords.addEventListener("input", () => {
    const count = Number(elements.wordCount.value) || 1;
    const loadedWords = parseCustomWords(elements.customWords.value).length;
    setWordHint(`Cargadas: ${loadedWords}. Necesarias: ${count}.`, loadedWords < count);
  });
